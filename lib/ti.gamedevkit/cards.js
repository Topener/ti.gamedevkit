...
function instance (conf) {
    ...
    var deck = [];
    var deckArgs;
    var config = {animationSpeed: 750, backImage: '/images/cardbacks/lines.jpg'};
     ...
     */
    this.dealDeckCardToPosition = function(y, z, flip) {
    ...
    this.setBackImage = function (cards, image) {
    ...
    */
    ...
    function flip() {
        ...
        cardView.animateTo = cardView.face === 'back' ? 'front' : 'back';
        cardView.face = cardView.animateTo;
        card.animate(Ti.UI.createAnimation({
            width: 1,
            duration: 250
        }), function () {
            if (cardView.animateTo == 'back') {
                cardView.front = card.image;
                card.image = self.getBack();
            } else {
                card.image = cardView.front;
            }
            delete cardView.animateTo;
            card.animate(Ti.UI.createAnimation({
                width: deckArgs.height / 1.42,
                duration: 250
            }));
